...
CERT_PATH="$DATA_DIR/ssl/live/$DOMAIN/fullchain.pem"
...
mkdir -p "$DATA_DIR/ssl" "$DATA_DIR/certbot/www" "$DATA_DIR/certbot/logs" "$DATA_DIR/ips"
...
if [ -d "$DATA_DIR/ssl/accounts" ] && [ ! -d "$DATA_DIR/ssl/live" ]; then
...
if [ "$CHALLENGE_TYPE" = "dns" ]; then
    ...
    cat > "$DATA_DIR/ssl/cloudflare.ini" << EOF
# Cloudflare API credentials - auto-generated by init-ssl.sh
dns_cloudflare_api_token = $CLOUDFLARE_API_TOKEN
EOF
    chmod 600 "$DATA_DIR/ssl/cloudflare.ini"
