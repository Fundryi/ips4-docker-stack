<?php
$baseRequirements = [ 'php' => [ 'required' => '8.0.0', 'recommended' => '8.1.0' ], 'mysql' => [ 'required' => '5.5.3', 'recommended' => '8.0.13' ] ]; // Base requirements for 4.7
$streamContext = stream_context_create( [ 'http' => [ 'user_agent' => 'Invision Community Requirement Checker' ] ] );
$remoteRequirements = json_decode( file_get_contents( 'https://remoteservices.invisionpower.com/requirements?version=107800', FALSE, $streamContext ), TRUE );
...
{
	$bits = explode( '.', $latestVersion['version'] );
	$majorVersion = $bits[0] . '.' . $bits[1];
}

// Make PHP 8.0 the minimum regardless of the remote version
if( $remoteRequirements AND version_compare( $remoteRequirements['php']['required'], '8.0.0', '<' ) )
{
	$remoteRequirements['php']['required'] = '8.0.0';
}
$requirements = $remoteRequirements ?: $baseRequirements;
...
<html>
	<head>
		<meta charset="utf-8"/>
		<title>Get Ready for Invision Community <?php echo $majorVersion; ?></title>
...
<?php if ( isset( $_GET['phpinfo'] ) ) { phpinfo(); exit; } ?>
		<header>
			...
			Invision Community <?php echo $majorVersion; ?>
		</header>
		<div id='main'>
<?php $success = TRUE; $installOnly = TRUE; ?>
			<section>
				...
				<ul>
<?php if ( version_compare( PHP_VERSION, '8.1.999' ) < 1 AND version_compare( PHP_VERSION, $requirements['php']['required'] ) >= 0 ): ?>
					<li class="success">PHP version <?php echo PHP_VERSION; ?>.</li>
<?php else: $success = FALSE; ?>
					<li class="fail">You are not running a compatible version of PHP. You need PHP <?php echo $requirements['php']['required']; ?> or <?php echo $requirements['php']['recommended']; ?>. You should contact your hosting provider or system administrator to ask for this to be changed.</li>
<?php endif; ?>
...
<?php else: ?>
					<li class="advisory">You do not have the Zip PHP extension loaded. While this is not required, it is recommended. You may wish to contact your hosting provider or system administrator to ask for it to be installed.</li>
<?php endif; ?>
<?php if ( extension_loaded( 'exif' ) ): ?>
					<li class="success">Exif extension loaded.</li>
<?php else: ?>
					<li class="advisory">You do not have the Exif PHP extension loaded. While this is not required, it is recommended. You may wish to contact your hosting provider or system administrator to ask for it to be installed.</li>
<?php endif; ?>
<?php if ( \function_exists('gmp_init') ): ?>
					<li class="success">GMP extension loaded.</li>
<?php else: ?>
...
<?php
	...
	$installOnly = FALSE;
?>
<?php if ( $mysql->connect_errno ): ?>
						<li class="advisory">MySQL connection could not be established to perform version check. Make sure your MySQL Server version is <?php echo $requirements['mysql']['required']; ?> or above (<?php echo $requirements['mysql']['recommended']; ?> or above recommended).</li>
<?php elseif ( version_compare( $mysql->server_info, $requirements['mysql']['recommended'] ) >= 0 or mb_strpos( $mysql->server_info, "MariaDB" ) ): ?>
						<li class="success">MySQL version <?php echo $mysql->server_info; ?>.</li>
<?php elseif ( version_compare( $mysql->server_info, $requirements['mysql']['required'] ) >= 0 ): ?>
						<li class="advisory">You are running MySQL version <?php echo $mysql->server_info; ?>.<p class='smaller'>While this version is compatible, we recommend version <?php echo $requirements['mysql']['recommended']; ?> or above. You may wish to contact your hosting provider or system administrator to ask for an upgrade if you are upgrading to Invision Community <?php echo $majorVersion; ?></p></li>
<?php else: $success = FALSE; ?>
						<li class="fail">You are not running a compatible version of MySQL. You need MySQL <?php echo $requirements['mysql']['required']; ?> or above (<?php echo $requirements['mysql']['recommended']; ?> or above recommended). You should contact your hosting provider or system administrator to ask for an upgrade.</li>
<?php endif; ?>

<?php  else: $mysql = @new my_mysqli( 'localhost' ); if ( $mysql->connect_errno ): ?>
						<li class="advisory">MySQL connection could not be established to perform version check. Make sure your MySQL Server version is <?php echo $requirements['mysql']['required']; ?> or above (<?php echo $requirements['mysql']['recommended']; ?> or above recommended).</li>
<?php else: ?>
<?php if ( version_compare( $mysql->server_info, $requirements['mysql']['recommended'] ) >= 0 ): ?>
						<li class="success">MySQL version <?php echo $mysql->server_info; ?>.</li>
<?php elseif ( version_compare( $mysql->server_info, $requirements['mysql']['required'] ) >= 0 ): ?>
						<li class="advisory">You are running MySQL version <?php echo $mysql->server_info; ?>. While this version is compatible, we recommend version <?php echo $requirements['mysql']['recommended']; ?> or above. You may wish to contact your hosting provider or system administrator to ask for an upgrade.</li>
<?php else: $success = FALSE; ?>
						<li class="fail">You are not running a compatible version of MySQL. You need MySQL <?php echo $requirements['mysql']['required']; ?> or above. You should contact your hosting provider or system administrator to ask for an upgrade.</li>
<?php endif; ?>
<?php endif; endif; ?>
					</ul>
				</section>
...
<?php endif; ?>
			...
			<section id="summary">
				<h1>Summary</h1>
<?php if ( $success ): ?>
				<p class="success">You are ready to install Invision Community <?php echo $majorVersion; ?>!</p>
<?php if ( $installOnly ): ?>
...
<?php endif; ?>
<?php else: ?>
				<p class="fail">You are not ready to upgrade to Invision Community <?php echo $majorVersion; ?> yet. See the information above for instructions how to fix or <a href="https://invisioncommunity.com/forums/" target="_blank">post on our community</a> for further assistance. If you're unable to fix the issues with your server, you may want to consider using <a href='https://invisioncommunity.com/buy'>Invision Cloud Community</a></p>
